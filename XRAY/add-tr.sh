...
proxies:
  - name: XRAY_TROJAN_TLS_${user}
    ...
    udp: true
proxy-groups:
  - name: Lone-Autoscript
    type: select
    proxies:
      - XRAY_TROJAN_TLS_${user}
      - DIRECT
rules:
  - MATCH,Lone-Autoscript
EOF

cat > /usr/local/etc/xray/$user-TROJAN-NTLS.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
cat > /usr/local/etc/xray/$user-TRCELCOM.yaml <<EOF
port: 7890
socks-port: 7891
redir-port: 7892
mixed-port: 7893
...
proxies:
  - name: XRAY_TROJAN_CELCOM_${user}
    ...
    network: ws
    ws-opts:
      path: wss://onlinepayment.celcom.com.my/trojanws
      headers:
        Host: ${domain}
    udp: true
proxy-groups:
...
mv /usr/local/etc/xray/$user-TRCELCOM.yaml /home/vps/public_html/$user-TRCELCOM.yaml
clear
echo -e ""
